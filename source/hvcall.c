...
static int install_hvsc_redirection(struct syscall_patch_ctx *ctx, int hvcall)
{
...
int
lv1_insert_htab_entry(uint64_t htab_id,
		      uint64_t hpte_group,
		      uint64_t hpte_v,
		      uint64_t hpte_r,
		      ...
		      uint64_t flags,
		      uint64_t * hpte_index,
		      uint64_t * hpte_evicted_v, uint64_t * hpte_evicted_r)
{
...
int
lv1_allocate_memory(uint64_t size,
		    uint64_t page_size_exp,
		    uint64_t flags, uint64_t * addr, uint64_t * muid)
{
	struct syscall_patch_ctx ctx;
	...
	__asm__ __volatile__("mr %%r3, %3;"
			     "mr %%r4, %4;"
			     "li %%r5, 0;" "mr %%r6, %5;" SYSCALL(SC_SLOT_2)
			     "mr %0, %%r3;"
			     "mr %1, %%r4;"
...
int
lv1_undocumented_function_114(uint64_t start,
			      uint64_t page_size,
			      uint64_t size, uint64_t * lpar_addr)
{
	struct syscall_patch_ctx ctx;
	...
	__asm__ __volatile__("mr %%r3, %2;"
			     "mr %%r4, %3;" "mr %%r5, %4;" SYSCALL(SC_SLOT_2)
			     "mr %0, %%r3;"
...
void *lv2_alloc(uint64_t size, uint64_t pool)
{
	struct syscall_patch_ctx ctx;
	static const uint64_t shell_code[] = {
		0x7C0802A67C140378ULL, 0x4BECB6317E80A378ULL,
