...
int
mm_insert_htab_entry(uint64_t va_addr,
		     uint64_t lpar_addr, uint64_t prot, uint64_t * index)
{
	uint64_t hpte_group;
...
int
mm_map_lpar_memory_region(uint64_t lpar_start_addr,
			  uint64_t ea_start_addr,
			  uint64_t size, uint64_t page_shift, uint64_t prot)
{
	int i;
	int result;
...
void *lv1_alloc(size_t sz, void *addr)
{
	uint64_t lpar, muid;
	int s = 1;
	int p = 12;
	...
	if (result) {
		PRINTF
		    ("lv1_allocate_memory(%d,%d,...) has failed with %08X :(\n",
		     s, p, result);
		return NULL;
	} else {
	...
	if (result) {
		PRINTF
		    ("mm_map_lpar_memory_region(%p,...) has failed with %08X :(\n",
		     (void *)lpar, result);
		return NULL;
	}
	return addr;
